<template>
  <el-card>
    <div class="flex align-items-center title">
      ...
      <div class="text-h3">Редактировать сотрудника</div>
    </div>
...
// Только владелец компании может редактировать сотрудников
...
const submit = () => {
  errors.value = {};
  apiFetch(`companies/${value.value.company_id}/employees/${value.value.id}`, {
    method: "PUT",
    body: value.value
  }).then(() => {
    router.push(`/profile/employees/${value.value.id}`)
  }).catch(({body}) => {
