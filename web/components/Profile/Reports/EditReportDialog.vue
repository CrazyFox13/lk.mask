    ...
    >
      <template #header="{close}">
        <div class="dialog-head">
          ...
          <div class="text-black modal-window-title text-center mb-10">Редактировать претензию</div>
        </div>
      </template>
...
const report = ref<IReport>(props.modelValue);
...
const submit = () => {
  errors.value = {};
  apiFetch(`reports/${report.value.id}`, {
    method: "put",
    body: report.value
  }).then(() => {
    emit('update:modelValue', report.value);
    handleClose();
  }).catch(({body}) => {
